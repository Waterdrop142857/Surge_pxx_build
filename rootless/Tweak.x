...
#import <dlfcn.h>                         // 动态库加载与符号解析，提供动态库加载和符号解析的接口。
...
#include <substrate.h>                    // Substrate：提供动态代码注入功能
...
// =============================== 辅助函数 =================================
// 作用：提供辅助功能，如 SHA256 计算

// NSString+SHA256 类别扩展
@interface NSString (SHA256)
- (NSData *)SHA256; // 返回 NSData 类型的 SHA256 哈希值
@end
...
- (NSData *)SHA256 {
    ...
    dispatch_once(&onceToken, ^{
        ...
        cache.countLimit = 3000; // 设置缓存上限，避免内存过度占用
    });
    ...
    // 缓存结果
    [cache setObject:hashData forKey:self];
    return hashData;
}
...
原 SGNSARequestHelper 的 request:completeBlock: 方法，被 SGRequestHelper 的 dataTaskWithURL:completionHandler: 和 dataTaskWithRequest:completionHandler: 方法取代。

SGRequestHelper 提供了更灵活的网络请求接口，包括 dataTaskWithURL:completionHandler: 和 dataTaskWithRequest:completionHandler: 方法。
实现的核心逻辑 :
请求构建：通过 NSMutableURLRequest 创建请求对象。
请求执行：调用 dataTaskWithRequest:completionHandler: 方法。
回调处理：支持 NSData *body、NSURLResponse *resp 和 NSError *err 参数。
...
// NSURLSession 是 Apple 提供的一个强大网络请求类，取代了老旧的 NSURLConnection。它负责实际的网络通信，比如发送 HTTP 请求、接收服务器响应、处理上传和下载任务等。它支持后台传输、暂停/恢复请求等高级功能，是 iOS/macOS 开发中网络请求的核心工具。
...
- (void)simulateRestoreSuccess {
    ...
    NSString *deviceID = @"fake_device_id"; // 默认值，可根据上下文动态获取

    // 生成伪造的 policy 数据，与 SGRequestHelper 中的逻辑保持一致
    NSData *keydata = [deviceID SHA256];
    const char *keybytes = [keydata bytes];
    char licEncOut[32] = {0};
    size_t encRet = 0;
...
- (void)_connect {
    if ([self respondsToSelector:@selector(setValue:forKey:)]) {
        Ivar coreStartedIvar = class_getInstanceVariable([self class], "_coreStarted");
        Ivar isReadyIvar = class_getInstanceVariable([self class], "_isReady");
        if (coreStartedIvar) {
            [self setValue:@YES forKey:@"_coreStarted"];
        } else {
            NSLog(@"[Error] _coreStarted not found");
        }
        if (isReadyIvar) {
            [self setValue:@YES forKey:@"_isReady"];
        } else {
            NSLog(@"[Error] _isReady not found");
        }
    }
    %orig;
}
...
- (void)setButtonBackgroundTo:(BOOL)connected;
- (void)updateUptime;
@end

%hook StartViewController
- (void)VPNManagerStatusChanged {
    if ([self respondsToSelector:@selector(setButtonBackgroundTo:)]) {
        [self setButtonBackgroundTo:YES]; // 更新按钮为“已连接”
    }
    if ([self respondsToSelector:@selector(updateUptime)]) {
        [self updateUptime]; // 更新运行时间
    }
    %orig;
}
...
- (void)configurationDidChange {
    %orig;
    [self VPNManagerStatusChanged]; // 配置变更后更新状态
}
...
%hook CFURLRequest
+ (CFURLRequest *)create:(CFURLRef)url {
    NSString *urlString = [(__bridge NSURL *)url absoluteString];
    // 更精确地拦截 Surge 服务器请求
    if ([urlString containsString:@"surge-activation.com"] || 
        [urlString containsString:@"nssurge.com"]) {
        return NULL; // 阻止特定请求
    }
    return %orig;
}
...
- (void)applicationWillEnterForeground:(UIApplication *)application {
    %orig;
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ResetSurgeState" object:nil];
}
%end



// Hook SGUProfileManager
%hook SGUProfileManager
- (void)scheduleProfileReload {
    // 阻止重载，不执行任何操作
}
...
- (void)sendRequest:(id)request completion:(void (^)(id response, NSError *error))completion {
    ...
    fakeResponse[@"license"] = @{
        @"email": @"pxx@gmail.com",
        @"expirationDate": @(FIXED_EXPIRATION_DATE),
        @"plan": @"lifetime",
        @"status": @"active"
    };
    fakeResponse[@"devices"] = @[@{@"id": @"1", @"name": [[UIDevice currentDevice] name]}];
    fakeResponse[@"inactive"] = @(0);
    if (completion) {
...
// 动态解析 IP 的集合
static NSMutableSet *surgeIPs = nil;
// Hook getaddrinfo 和 connect
static NSMutableSet *surgeIPs;
...
int hooked_getaddrinfo(const char *nodename, const char *servname, const struct addrinfo *hints, struct addrinfo **res) {
    int ret = original_getaddrinfo(nodename, servname, hints, res);
    if (ret == 0 && res && *res) {
        if (strstr(nodename, "surge-activation.com") || strstr(nodename, "surge.com") || strstr(nodename, "nssurge.com")) {
            struct addrinfo *addr = *res;
            while (addr) {
                if (addr->ai_family == AF_INET) {
                    char ip[INET_ADDRSTRLEN];
                    inet_ntop(AF_INET, &((struct sockaddr_in *)addr->ai_addr)->sin_addr, ip, INET_ADDRSTRLEN);
                    [surgeIPs addObject:[NSString stringWithUTF8String:ip]];
                }
                addr = addr->ai_next;
            }
        }
...
int hooked_connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    if (addr->sa_family == AF_INET) {
        ...
        NSString *destIP = [NSString stringWithUTF8String:ip];
        if ([destIP containsString:@"surge"] || [destIP containsString:@"nssurge"]) {
            errno = ECONNREFUSED;
            return -1; // 拒绝连接
        }
    }
...
// Hook libboringssl.dylib 中的 EVP_DigestVerifyFinal
int hooked_lib_EVP_DigestVerifyFinal(void *ctx, const unsigned char *sig, size_t siglen) {
    return 1; // 绕过验证
}

// Hook libboringssl.dylib 中的 EVP_DigestVerify
int hooked_EVP_DigestVerify(void *ctx, const unsigned char *sig, size_t siglen) {
    return 1; // 绕过验证
}

// Hook libboringssl.dylib 中的 SSL_do_handshake
static int (*original_SSL_do_handshake)(void *);
int hooked_SSL_do_handshake(void *ssl) {
    int result = original_SSL_do_handshake ? original_SSL_do_handshake(ssl) : 1;
    if (result <= 0) {
        return 1; // 如果握手失败，返回成功
    }
    return result;
}
...
// %ctor 中初始化所有 Hook，确保在应用启动时生效
%ctor {
    // --- 第一步：优先反制越狱检测 ---
    // 拦截越狱检测
    MSHookFunction(dlopen, hooked_dlopen, (void **)&original_dlopen);   // 拦截动态库加载
    MSHookFunction(getenv, hooked_getenv, (void **)&original_getenv);   // 伪装环境变量
    ...
    // Surge 主程序特征码（36 字节）
    unsigned char needle_surge[] = {0xFF, 0x83, 0x02, 0xD1, 0xF8, 0x5F, 0x06, 0xA9,
                                   0xF6, 0x57, 0x07, 0xA9, 0xF4, 0x4F, 0x08, 0xA9,
                                   0xFD, 0x7B, 0x09, 0xA9, 0xFD, 0x43, 0x02, 0x91,
                                   0xF3, 0x03, 0x02, 0xAA, 0xF4, 0x03, 0x01, 0xAA,
                                   0xF5, 0x03, 0x00, 0xAA};
    ...
    unsigned char needle_lib[] = {
        0xFF, 0xC3, 0x02, 0xD1, 0xF6, 0x57, 0x08, 0xA9, 0xF4, 0x4F, 0x09, 0xA9, 0xFD, 0x7B, 0x0A, 0xA9,
        0xFD, 0x83, 0x02, 0x91, 0x48, 0x3F, 0x2A, 0xB0, 0x08, 0xF9, 0x42, 0xF9, 0x08, 0x01, 0x40, 0xF9,
        0xA8, 0x83, 0x1D, 0xF8, 0x08, 0x08, 0x40, 0xF9, 0x08, 0x01, 0x40, 0xF9, 0x08, 0x1D, 0x40, 0xF9,
        0x48, 0x04, 0x00, 0xB4, 0xF3, 0x03, 0x02, 0xAA, 0xF4, 0x03, 0x01, 0xAA, 0xF5, 0x03, 0x00, 0xAA
    };
    ...
    unsigned char needle_EVP_DigestVerify[] = {
        0xF6, 0x57, 0xBD, 0xA9, 0xF4, 0x4F, 0x01, 0xA9, 0xFD, 0x7B, 0x02, 0xA9, 0xFD, 0x83, 0x00, 0x91,
        0xF3, 0x03, 0x02, 0xAA, 0xF4, 0x03, 0x01, 0xAA, 0xF5, 0x03, 0x00, 0xAA, 0x00, 0x08, 0x40, 0xF9,
        0x08, 0x00, 0x40, 0xF9, 0x09, 0x1D, 0x40, 0xF9, 0xA9, 0x01, 0x00, 0xB4, 0xE0, 0x03, 0x15, 0xAA,
        0xE1, 0x03, 0x03, 0xAA, 0xE2, 0x03, 0x04, 0xAA, 0x1B, 0xFF, 0xFF, 0x97, 0x00, 0x03, 0x00, 0x34
    };
    ...
    unsigned char needle_SSL_do_handshake[] = {
        0xFF, 0xC3, 0x00, 0xD1, 0xF4, 0x4F, 0x01, 0xA9, 0xFD, 0x7B, 0x02, 0xA9, 0xFD, 0x83, 0x00, 0x91,
        0xF3, 0x03, 0x00, 0xAA, 0x08, 0x18, 0x40, 0xF9, 0x1F, 0xBD, 0x00, 0xB9, 0x9F, 0x2A, 0x00, 0x94,
        0xDA, 0x2A, 0x00, 0x94, 0x68, 0x16, 0x40, 0xF9, 0x68, 0x04, 0x00, 0xB4, 0x68, 0x1A, 0x40, 0xF9,
        0x00, 0x89, 0x40, 0xF9, 0xC0, 0x03, 0x00, 0xB4, 0x08, 0xA0, 0x58, 0x39, 0x88, 0x03, 0x18, 0x37
    };
    ...
    if (funcAddr_ssl) {
        MSHookFunction(funcAddr_ssl, (void *)hooked_SSL_do_handshake, NULL);
    }

    // Hook connect 函数
    MSHookFunction((void *)connect, (void *)hooked_connect, (void **)&original_connect);
}